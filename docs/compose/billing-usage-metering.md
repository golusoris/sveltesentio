...
billable signal), **overage forecasting** (alert _before_ the bill
         ...
         WHERE tenant_id = $1 AND unit = $2
           AND window_start >= $3 AND window_end <= $4`,
			...
			if (meter.overage.hardLimit && Number(used.total) >= meter.overage.hardLimit) {
				await db.query(`UPDATE tenants SET status = 'usage_blocked' WHERE id = $1`, [t.tenant_id]);
				await sendNotification({
					tenantId: t.tenant_id,
...
4. **Aggregating the _current_ window** — events still arriving;
