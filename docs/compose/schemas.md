...
(drop unknown but don't fail). For API surfaces we want the _fail_ behaviour
...
- **`z.object({ … }).partial()` as a PATCH schema.** Use `.partial()` only
  for intermediate types; API-surface PATCH schemas usually require _at
  least one_ field — use `.refine((v) => Object.keys(v).length > 0)`.
