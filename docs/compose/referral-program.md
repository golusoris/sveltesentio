...
export async function issueCodeForReferrer(accountId: string, programId: string) {
	...
	const existing = await db
		...
		.from(referralCode)
		.where(
			and(eq(referralCode.referrerAccountId, accountId), eq(referralCode.programId, programId)),
		)
		.limit(1);
...
export async function markQualifyingEvent(
	refereeAccountId: string,
	kind: 'paid_invoice' | 'retention_30d',
) {
...
<section aria-labelledby="your-stats">
	...
	<dl>
		<dt>Invited</dt>
		<dd>{data.stats.totalSignups}</dd>
		<dt>Qualified</dt>
		<dd>{data.stats.qualified}</dd>
		<dt>Pending</dt>
		<dd>{data.stats.pending}</dd>
		<dt>Earned</dt>
		<dd>{data.stats.totalEarnedMinor / 100}€</dd>
	</dl>
</section>
