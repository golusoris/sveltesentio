...
export type YjsTextObserver = (event: Y.YTextEvent, transaction: Y.Transaction) => void;
...
export function observeYjsText(yText: Y.Text, listener: YjsTextObserver): () => void {
	yText.observe(listener);
	return () => yText.unobserve(listener);
}

export function insertYjsText(yText: Y.Text, index: number, value: string): void {
	if (value.length === 0) return;
	yText.insert(index, value);
}

export function deleteYjsText(yText: Y.Text, index: number, length: number): void {
	if (length <= 0) return;
