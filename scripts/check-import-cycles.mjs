...
// specifiers and resolves relative ones against the filesystem; it does not run
// the TypeScript resolver. That misses dynamic `import()` and path aliases, which
// is recorded as the gap in .config/hiss/coverage.yaml rather than implied to be
// covered.
...
function specifiersOf(source) {
	const found = new Set();
	for (const re of [SPECIFIER, BARE]) {
		re.lastIndex = 0;
		let match;
